...
- **The world is one git repository** (§5.2): the kernel commits it at the end of every node turn with the git author set to the node's uid, so authorship in git *is* authorship in the society — the raw truth of the git layer of the monitoring pipeline ([`research/monitoring-architecture.md`](research/monitoring-architecture.md)). Three mandatory safeguards (documented incidents): **refuse a world = `$HOME` or a broad root** (Codex incident: 200 GB); **`.git` masked read-only in the sandbox** (otherwise the agent can erase the trace of its own mutations); periodic gc. The world repo is **distinct** from the project repo — the project's chronicle does not depend on the agents' world. `core.untrackedCache=true`, no FSMonitor (unavailable on Linux). Measured commit cost 29-100 ms; at one commit per turn the whole class of filesystem-watcher bugs disappears by construction, since HEAD moves only when the kernel moves it.
