...
import {
  mkdtemp, rm, open as openFile, readFile, appendFile, stat,
...
async function countSyncs<T>(fn: () => Promise<T>): Promise<{ result: T; syncs: number }> {
...
describe('fsync discipline', () => {
  it('fsyncs at least once on every successful flush', async () => {
    ...
    const { syncs } = await countSyncs(() => w.flush());
  ...
  it('fsyncs after a repair that actually truncates', async () => {
    ...
    const { result, syncs } = await countSyncs(() => repair(home, 'n1'));
  ...
  it('does not fsync on a repair that finds nothing to truncate', async () => {
    ...
    w.append('test/ping', { n: 0 });
    await w.close();
    const { syncs } = await countSyncs(() => repair(home, 'n1'));
    expect(syncs).toBe(0);
  });
});
...
describe('claim-check byte boundaries', () => {
  ...
  function payloadOfCanonicalBytes(bytes: number): { payload: string } {
